...
import { BarChart3, Download, Lightbulb, Calendar, Heart, Clock, Armchair, User, Bed } from 'lucide-react';
...
function downloadCSV(readings: Reading[], startDate?: string, endDate?: string) {
  ...
  // CSV header
  const headers = ['id', 'date', 'time', 'systolic', 'diastolic', 'pulse', 'body_position', 'note'];
...
function calculateStats(
  readings: Reading[],
  days: number
): { systolic: number; diastolic: number; pulse: number; count: number } | null {
  const cutoffDate = new Date();
  cutoffDate.setDate(cutoffDate.getDate() - days);

  const recentReadings = readings.filter((r) => new Date(r.timestamp) >= cutoffDate);
  ...
  return {
    ...
    pulse: pulseReadings.length > 0 ? Math.round(totalPulse / pulseReadings.length) : 0,
    count: recentReadings.length,
  };
}

function getBPCategory(systolic: number, diastolic: number): { label: string; color: string; message: string } {
  ...
  // Check for Stage 1
  else if ((systolic >= 130 && systolic <= 139) || (diastolic >= 82 && diastolic <= 89)) {
    return {
      label: 'High Blood Pressure (Stage 1)',
      color: '#ef4444',
      message: 'You have stage 1 hypertension. Consult with your healthcare provider.',
    };
  }
  // Check for Elevated
    ...
    return {
      label: 'Elevated',
      color: '#f59e0b',
      message: 'Your blood pressure is elevated. Consider lifestyle modifications.',
    };
  }
  // Normal
    ...
    return {
      label: 'Normal',
      color: '#10b981',
      message: 'Your blood pressure is in the normal range. Keep up the good work!',
    };
  }
  ...
  const statsAllTime = {
    ...
    pulse: (() => {
      ...
      return withPulse.length > 0
        ? Math.round(withPulse.reduce((sum, r) => sum + (r.pulse || 0), 0) / withPulse.length)
        : 0;
    })(),
        ...
        <div style={styles.card}>
          <div style={styles.emptyState}>
            ...
            <p style={styles.emptyText}>
              Add some readings to see your blood pressure summary and export data
            </p>
          </div>
        </div>
      </div>
          ...
          <div style={{
            ...
            <div style={{...styles.latestBP, gap: '4px'}}>
              <div style={styles.bpNumbersRow}>
                <span style={{...styles.latestValue, color: category.color}}>{lastReading.systolic}</span>
                <span style={styles.bpDivider}>/</span>
                <span style={{...styles.latestValue, color: category.color}}>{lastReading.diastolic}</span>
              </div>
            ...
            <div style={styles.latestInfoGrid}>
              {lastReading.pulse && (
                <div style={styles.latestInfoItem}>
                  <div style={{...styles.latestInfoIcon, backgroundColor: '#fce7f3'}}>
                    <Heart size={16} color="#ec4899" />
                  </div>
                  <div style={styles.latestInfoText}>
                    <span style={styles.latestInfoLabel}>Pulse</span>
                    <span style={styles.latestInfoValue}>{lastReading.pulse} bpm</span>
                  </div>
                </div>
              )}
              
              <div style={styles.latestInfoItem}>
                <div style={{...styles.latestInfoIcon, backgroundColor: '#e0e7ff'}}>
                  {lastReading.bodyPosition === 'seated' && <Armchair size={16} color="#6366f1" />}
                  {lastReading.bodyPosition === 'leaning' && <User size={16} color="#6366f1" />}
                  {lastReading.bodyPosition === 'laying' && <Bed size={16} color="#6366f1" />}
                </div>
                <div style={styles.latestInfoText}>
                  <span style={styles.latestInfoLabel}>Position</span>
                  <span style={styles.latestInfoValue}>
                    {lastReading.bodyPosition.charAt(0).toUpperCase() + lastReading.bodyPosition.slice(1)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div style={{
              ...
              <p style={{...styles.categoryMessage, color: category.color, fontSize: '12px', margin: 0}}>{category.message}</p>
            </div>

            {/* Note if exists */}
            {lastReading.note && (
              <div style={{...styles.latestNote, padding: '12px', marginTop: '4px'}}>
      ...
      <div style={styles.statsGrid}>
        {stats7Days && (
          <div style={{
            ...
            <div style={styles.statSecondary}>
              {stats7Days.pulse > 0 && (
                <span style={styles.statSubValue}>Avg Pulse: {stats7Days.pulse} bpm</span>
              )}
              <span style={styles.statSubValue}>{stats7Days.count} readings</span>
            </div>
          </div>
        )}
        ...
        {stats30Days && (
          <div style={{
            ...
            <div style={styles.statSecondary}>
              {stats30Days.pulse > 0 && (
                <span style={styles.statSubValue}>Avg Pulse: {stats30Days.pulse} bpm</span>
              )}
              <span style={styles.statSubValue}>{stats30Days.count} readings</span>
            </div>
          </div>
        )}
        ...
        {stats90Days && (
          <div style={{
            ...
            <div style={styles.statSecondary}>
              {stats90Days.pulse > 0 && (
                <span style={styles.statSubValue}>Avg Pulse: {stats90Days.pulse} bpm</span>
              )}
              <span style={styles.statSubValue}>{stats90Days.count} readings</span>
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          <div style={styles.statSecondary}>
            {statsAllTime.pulse > 0 && (
              <span style={styles.statSubValue}>Avg Pulse: {statsAllTime.pulse} bpm</span>
            )}
            <span style={styles.statSubValue}>{statsAllTime.count} total readings</span>
          </div>
        </div>
      </div>
      ...
      <div style={styles.card}>
        ...
        <div style={styles.exportSection}>
          <p style={styles.exportDescription}>
            Export your blood pressure readings as a CSV file for use with spreadsheet software or
          ...
          <div style={styles.exportInfo}>
            <p style={styles.exportInfoText}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <Lightbulb size={16} />
                The CSV file will include: Date, Time, Systolic, Diastolic, Pulse, Body Position,
                and Notes
              </span>
            </p>
          </div>
