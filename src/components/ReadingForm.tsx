...
export function ReadingForm({ onSubmit, editingReading, onCancel }: ReadingFormProps) {
  const [date, setDate] = useState('');
  const [time, setTime] = useState('');
  const [systolic, setSystolic] = useState('');
  const [diastolic, setDiastolic] = useState('');
  const [pulse, setPulse] = useState('');
  const [bodyPosition, setBodyPosition] = useState<'seated' | 'leaning' | 'laying'>('seated');
    ...
    <div style={styles.card} className="form-slide-in">
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        <div style={styles.grid}>
          ...
          <div style={styles.formGroup}>
            ...
            <select
              id="bodyPosition"
              value={bodyPosition}
              onChange={(e) => setBodyPosition(e.target.value as 'seated' | 'leaning' | 'laying')}
              style={styles.select}
              aria-required="true"
            >
        ...
        <div style={styles.actions}>
          ...
          {editingReading && (
            <button type="button" onClick={handleCancelClick} style={styles.buttonSecondary}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <X size={18} />
                Cancel
              </span>
